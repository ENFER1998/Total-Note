...
function toggleMenu() {
  ...
  document.getElementById('menu-overlay').classList.toggle('mostrar');
}
function cerrarMenu() {
  document.getElementById('sidebar').classList.remove('open');
  document.getElementById('menu-overlay').classList.remove('mostrar');
}
function abrirModal(id) { cerrarMenu(); document.getElementById(id).classList.add('mostrar'); }
...
function actualizarDashboard(datos) {
  ...
  if (datos.gastosDashboard && datos.gastosDashboard.length > 0) {
    let maxGasto = datos.gastosDashboard[0].total; // Para el 100% de la barra
    datos.gastosDashboard.forEach(g => {
      let pct = (g.total / maxGasto) * 100;
      contGastos.innerHTML += `
        <div style="margin-bottom:12px;">
          <div style="display:flex; justify-content:space-between; font-size:0.85rem; font-weight:bold; color:var(--primary);">
            <span>${g.categoria}</span><span style="color:var(--danger);">${moneda.format(g.total)}</span>
          </div>
          <div style="background:#e2e8f0; border-radius:5px; height:8px; width:100%; margin-top:5px;">
  ...
  if (datos.listaInventario && datos.listaInventario.length > 0) {
    datos.listaInventario.forEach(item => {
      if(item.stock > 0) ulStock.innerHTML += `<li><div class="lista-texto"><strong>${item.desc}</strong><br>${moneda.format(item.precio)}</div> 
          <div style="display:flex; gap:5px; align-items:center;"><span class="badge-stock">${item.stock}</span><button class="btn-edit" onclick="abrirEditInv('${item.id}', '${item.tipo}', '${limpiarTexto(item.desc)}', ${item.costo}, ${item.precio}, ${item.stock}, ${item.minimo})"><i class="fas fa-pen"></i></button></div></li>`;
    });
  ...
  if (datos.listaTaller && datos.listaTaller.length > 0) {
    datos.listaTaller.forEach(item => {
      ulTaller.innerHTML += `<li><div class="lista-texto"><strong>${item.cliente}</strong><br>${item.equipo}</div> 
        <div style="display:flex; gap:5px;"><button onclick="abrirCobro('${item.id}', '${limpiarTexto(item.cliente)}', '${limpiarTexto(item.equipo)}', ${item.presupuesto})" style="background:var(--success); color:white; border:none; padding:8px 10px; border-radius:8px; font-weight:bold; cursor:pointer;">Cobrar</button>
        <button class="btn-edit" onclick="abrirEditTaller('${item.id}', '${limpiarTexto(item.cliente)}', '${limpiarTexto(item.equipo)}', '${limpiarTexto(item.falla)}', ${item.presupuesto})"><i class="fas fa-pen"></i></button></div></li>`;
    });
  ...
  if (datos.listaDeudas && datos.listaDeudas.length > 0) {
    datos.listaDeudas.forEach(item => {
      ...
      ulDeudas.innerHTML += `<li><div class="lista-texto"><strong style="${colorVenc}">${item.acreedor}</strong><br><span style="font-size:0.8rem; color:#64748b;">${item.cuotaInfo} - ${textoVenc}</span></div>
        <div style="display:flex; gap:5px;"><button onclick="abrirPagoDeuda(${item.fila}, '${limpiarTexto(item.acreedor)}', ${item.monto})" style="background:var(--accent); color:white; border:none; padding:8px 10px; border-radius:8px; font-weight:bold; cursor:pointer;">Pagar</button>
        <button class="btn-edit" onclick="abrirEditDeuda(${item.fila}, '${limpiarTexto(item.acreedor)}', '${item.montoTotal}', '${item.montoCuota}', '${item.cuotaActual}', '${item.cuotasTotales}', '${item.diaVenc}')"><i class="fas fa-pen"></i></button></div></li>`;
    });
  } else { ulDeudas.innerHTML = "<li>No hay obligaciones registradas.</li>"; }

  // 5. Alertas
  let lista = document.getElementById('lista-alertas'); lista.innerHTML = '';
  let hayAlertas = false;
  if (datos.stockBajo && datos.stockBajo.length > 0) { datos.stockBajo.forEach(aviso => { lista.innerHTML += `<li><span><i class="fas fa-box" style="color:#f59e0b"></i> ${aviso}</span><span class="texto-rojo">Comprar</span></li>`; hayAlertas = true; }); }
  if (datos.alertasDeuda && datos.alertasDeuda.length > 0) {
    datos.alertasDeuda.forEach(deuda => {
      ...
      lista.innerHTML += `<li><span><i class="fas fa-file-invoice-dollar" style="color:#ef4444"></i> ${deuda.acreedor} ${deuda.cuotaInfo}</span><span class="texto-rojo">${estadoTxt}</span></li>`; hayAlertas = true;
    });
  }
...
function abrirCobro(id, cli, eq, pres) { document.getElementById('cob-id').value = id; document.getElementById('cob-info').value = cli + " - " + eq; document.getElementById('cob-monto').value = pres; cerrarModal('modal-lista-taller'); abrirModal('modal-cobro'); }
